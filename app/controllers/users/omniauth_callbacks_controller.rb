class Users::OmniauthCallbacksController < Devise::OmniauthCallbacksController
  def twitter
    ...
    unless ['publisher', 'editor'].include? role
      redirect_to root_path, alert: 'Invalid role' and return
    end
    ...
    if user.persisted?
      ...
      if user.roles.first.name != role
        redirect_to root_path, alert: "You are already #{user.roles.first.name}, you cannot login as #{role}. Please get in touch with administrator if you would like to change your profile." and return
      end
